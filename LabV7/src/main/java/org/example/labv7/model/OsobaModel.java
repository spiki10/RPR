...
public class OsobaModel {
    ...
    public OsobaModel() {
        ...
        dateFormat = new SimpleDateFormat("yyyy-MM-dd"); // Update the date format to match the file
    }
    ...
    public String obrisiOsobu(Integer id)
    {
        for(Osoba osoba : osobe){
            if(osoba.getId() == id){
                osobe.remove(osoba);
                return "Osoba je uspjesno obirsana";
            }
        }
        return "Osoba nije pronadjena!";
    }
    ...
    public void napuniPodatkeIzTxtDatoteke(String putanjaDoDatoteke) throws IOException, ParseException, FileNotFoundException {
        osobe = FXCollections.observableArrayList();
